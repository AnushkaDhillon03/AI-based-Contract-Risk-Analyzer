...
if "analysis_result" in st.session_state:
    ...
    with tab_eval:
        ...
        if not evaluation:
            ...
            combined = evaluation.get("combined_score", {})
            overall = combined.get("combined_percentage", 0)
...
Clause: {combined.get("clause_percentage",0)}% |
Summary: {combined.get("summary_percentage",0)}%
</div>
</div>
''', unsafe_allow_html=True)
            ...
            with st.expander("Clause Evaluation", expanded=False):
                ...
                for item in evaluation["clause_evaluation"]["criteria"]:
            ...
            if "summary_evaluation" in evaluation:
                with st.expander("Summary Evaluation", expanded=False):
                    ...
                    for item in evaluation["summary_evaluation"]["criteria"]:
                        pct=0 if item["max_score"]==0 else item["score"]/item["max_score"]
            ...
            with st.expander("Combined Scores", expanded=False):
                ...
                c1.metric("Clause Score",f"{combined.get('clause_percentage',0)}%")
                c2.metric("Summary Score",f"{combined.get('summary_percentage',0)}%")
                c3.metric("Combined Score",f"{combined.get('combined_percentage',0)}%")
